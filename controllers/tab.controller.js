...
module.exports = {
  getAllTab: async(req, res, next) => {
    try {
      ...
      console.log('dfsdflang',lang)
      // if ( lang === 'en') {

        // const tablang = await Tab.find({"names.lang": "en"}, {names: {$elemMatch: {lang: "en"}}}); //อันอื่นไม่มาด้วย
        // const tablang = await Tab.find( {}, { names: { $elemMatch: { lang: "en"  } } } ) //อันอื่นไม่มาด้วยเหมือนกัน
        // const tablang = await Tab.find( { name: { lang: "en" } } )

      //   console.log(tablang);
      //   res.status(200).json(tablang)
